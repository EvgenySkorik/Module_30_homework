...
def test_create_client(client):
    ...
    client_data = {"name": "Лев", "surname": "Толстой",
                   "credit_card": "1455 1211 1489 0006", "car_number": "ТЕЛЕГА"}
...
def test_create_parking(client):
    ...
    parking_data = {"address": "Крассная д.2", "opened": True,
                    "count_places": 3, "count_available_places": 3}
...
def test_parking_in_success(client, db):
    ...
    resp = client.post("/api/clients_parkings/to/", json={"client_id": 1, "parking_id": 1})
    assert resp.status_code == 201
...
def test_parking_out_success(client, db):
    ...
    resp = client.put("/api/clients_parkings/out/", json={
        "client_id": 1,
        "parking_id": 1
    })
